...
const OrderDetail = (): JSX.Element => {
  ...
  const printAppRef = React.useRef<HTMLInputElement>(null)
  const printActRef = React.useRef<HTMLInputElement>(null)
  const printCommentRef = React.useRef<HTMLInputElement>(null)
  ...
  const downloadDocument = (id: string) => {
    html2canvas(document.getElementById(`${id}Id`) as HTMLElement).then(
      (canvas) => {
        ...
        } else if (id === 'actOfSelecion') {
          pdf.save(`Акт отбора проб № ${actDetail.id}.pdf`)
        } else {
          pdf.save(`Комментарии к заявке № ${data.id}.pdf`)
        }
      },
    )
  }
  ...
  const printDocument = useReactToPrint({
    content: () => printAppRef.current,
  })

  const printActOfSelection = useReactToPrint({
    content: () => printActRef.current,
  })

  const printComment = useReactToPrint({
    content: () => printCommentRef.current,
  })
          ...
          <CCard className="px-0">
            ...
            >
              <ButtonComponent
                id={1}
                name="Скачать"
                func={() => downloadDocument('application')}
              />
              <ButtonComponent
                id={2}
                name="Печать"
            ...
            <CCard className="mt-4 px-0">
              ...
              >
                <ButtonComponent
                  id={1}
                  name="Скачать"
                  func={() => downloadDocument('actOfSelecion')}
                />
                <ButtonComponent
                  id={2}
                  name="Печать"
                  func={() => printActOfSelection()}
                />
              </div>
            </CCard>
          ) : (
